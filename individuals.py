...
def evaluate_model(X, y, class_weights, aucs, n_folds=10, X_val=None, y_val=None):
    """
    The function `evaluate_model` performs k-fold cross-validation on a given dataset, trains a model
    using a BiLSTM architecture, and evaluates the model's performance using various metrics.
        
    X: The input features for the model
    y: The target variable or the labels for the classification problem. It should be a 1-dimensional array or list
...
model=None
model = bilstm()
history = model.fit(X_train,y_train,batch_size=BATCH,epochs=EPOCHS,validation_split=VS,callbacks=[es], shuffle=True
        ,class_weight=class_weights1)
model.save('model1.h5')

model=None
model = bilstm()
history = model.fit(X2_train,y2_train,batch_size=BATCH,epochs=EPOCHS,validation_split=VS,callbacks=[es], shuffle=True
        ,class_weight=class_weights2)
model.save('model2.h5')

model=None
model = bilstm()
history = model.fit(X3_train,y3_train,batch_size=BATCH,epochs=EPOCHS,validation_split=VS,callbacks=[es], shuffle=True
